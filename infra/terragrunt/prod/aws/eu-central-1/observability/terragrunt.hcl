...
dependency "alb" {
  ...
  mock_outputs = {
    alb_arn_suffix              = "app/todo-list-prod-api/0000000000000000"
    api_target_group_arn_suffix = "targetgroup/todo-list-prod-api/0000000000000000"
  }
}
...
dependency "api_service" {
  ...
  mock_outputs_allowed_terraform_commands = ["init", "validate", "plan"]
  mock_outputs = {
    service_name = "todo-list-prod-api"
  }
}
...
dependency "ingestion_service" {
  ...
  mock_outputs_allowed_terraform_commands = ["init", "validate", "plan"]
  mock_outputs = {
    service_name = "todo-list-prod-ingestion"
  }
}
...
dependency "database" {
  ...
  mock_outputs_allowed_terraform_commands = ["init", "validate", "plan"]
  mock_outputs = {
    cluster_identifier = "todo-list-prod-docdb"
  }
}
...
dependency "redis" {
  ...
  mock_outputs_allowed_terraform_commands = ["init", "validate", "plan"]
  mock_outputs = {
    replication_group_id = "todo-list-prod-redis"
  }
}
...
inputs = {
  ...
  ecs_service_min_running_tasks = {
    (dependency.api_service.outputs.service_name)       = 2
